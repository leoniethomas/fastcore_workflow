function [contextSpecificModel, retainedRxns, indicesCompletedCoreOrig, paramsForPipeline] = rFastcormicsPipeline(model, discretized, rownames, dico, biomassReactionName, consensusProportion, epsilon, optionalSettings, fillingMediumFlag, adaptiveScalingFlag)
...
if ~isempty(optionalSettings)
    paramsForPipeline.optionalSettings = optionalSettings;
end
...
if ~isempty(optionalSettings) && isfield(optionalSettings, 'medium')
    ...
    %paramsForPipeline.referenceModel = mediumConstrainedModel; 
...
elseif isempty(optionalSettings)
    ...
    %paramsForPipeline.referenceModel = origModel;
...
if sum(mapping) == 0
    ...
    paramsForPipeline.mapping = mapping;
end
...
paramsForPipeline.coreReactions = rxnNamesCompletedCore;
...
paramsForPipeline.contextSpecificModel = contextSpecificModel;
end
