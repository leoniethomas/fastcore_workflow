function solution = optimizeCbModelParams(model, params)
...
%
%                         * g0 - `n x 1` weights on zero norm, where positive is minimisation,
%                                negative is maximisation, zero is neither
%                         * g1 - `n x 1` weights on one norm, where positive is minimisation,
%                                negative is maximisation, zero is neither
%                         * g2 - `n x 1` weights on two norm
%
%    params:           (optional) structure containing solver and optimization parameters:
%
%                         * osenseStr   - Maximize ('max') / minimize ('min') (default = 'max')
%                                         Linear part of the objective. Nonlinear parts of the
%                                         objective are always assumed to be minimised.
%
%                         * minNorm     - {(0), 'one', 'zero', > 0 , n x 1 vector, 'optimizeCardinality'},
%                                         where `[m,n] = size(S)`;
%                                         0 - Default, normal LP
...
%                                         Six approximations are available: capped-L1 norm,
%                                         exponential function, logarithmic function, SCAD function,
%                                         L_p norm with p<0, L_p norm with 0<p<1.
%
%                                         Note: capped-L1, exponential and logarithmic function often
%                                         give the best results in terms of sparsity.
%
...
%                                         model.g0 - `n x 1` weights on zero norm, where positive is
%                                         minimisation, negative is maximisation, zero is neither.
%
...
%                                                 ~& lb \leq v \leq ub
%
%                                         `n x 1` Forms the diagonal of the positive definite
...
%                                         See `addLoopLawConstraints.m` for more info.
%
%                         * zeroNormApprox - approximation type of zero-norm (only available when minNorm = 'zero')
%                                             (default = 'cappedL1')
%
...
%
%                         * verify     - verify that the input fields are consistent
%                                         (default: false)
%
% OUTPUT:
...
%       - Vanille Lejal         20/01/26 Input handling to support a unified parameter struct (params)
%
% NOTE:
%
...
if isfield(params, 'osenseStr')

    % Retrieve value
    osenseStr = params.osenseStr;

    % Check that it is either "min" or "max"
    if ischar(osenseStr) || isstring(osenseStr)
        osenseStr = lower(string(osenseStr));

        if ~ismember(osenseStr, ["min", "max"])
            error('osenseStr must be either "min" or "max". Current value: %s', string(osenseStr));
        end

        % Normalize as char
        model.osenseStr = char(osenseStr);
    else
        error('osenseStr must be a string or char. Current type: %s', class(osenseStr));
    end
    
    % --- Remove the field from params ---
    params = rmfield(params, 'osenseStr');

elseif ~isfield(model, 'osenseStr')
...
if isfield(params, 'minNorm')
    ...
    % Retrieve value from the structure
    minNorm = params.minNorm;

    % --- Normalize booleans (logical or explicit "true"/"false" text only) ---
    if islogical(minNorm)
        isTrue = minNorm;

    elseif ischar(minNorm) || isstring(minNorm)
        strVal = lower(string(minNorm));
        if strVal == "true"
            isTrue = true;
        elseif strVal == "false"
            isTrue = false;
        else
            isTrue = [];  % not a boolean string ("zero", "min", "max", etc.)
        end
    ...
    % Replace 0 with [] to clearly distinguish cases
    if isequal(minNorm, 0)
        minNorm = [];
    end

    % If minNorm = "zero", handle approximation method
    if strcmpi(string(minNorm), "zero")

        if isfield(params, 'zeroNormApprox')
            zeroNormApprox = params.zeroNormApprox;
            ...
            % params = rmfield(params, 'zeroNormApprox'); % optional cleanup
        else
            zeroNormApprox = 'cappedL1';
        end
    end

    % --- Remove the field from params after using it ---
...
% allowLoops
if isfield(params, 'allowLoops')
    val = params.allowLoops;
    ...
    elseif isnumeric(val)
        if ismember(val, [0, 1])
            allowLoops = logical(val);  % convert to logical
        else
            error('allowLoops numeric value must be 0 or 1. Current value: %g', val);
        end

    % if string / char "true" or "false" ---
    elseif ischar(val) || isstring(val)
        strVal = lower(string(val));
        if strVal == "true"
            allowLoops = true;
        elseif strVal == "false"
            allowLoops = false;
        else
            error('allowLoops string value must be "true" or "false". Current value: %s', val);
        end

    % Any other type is invalid
    else
        error('allowLoops must be logical, numeric (0/1), or string ("true"/"false"). Current type: %s', class(val));
    end
    
    params = rmfield(params, 'allowLoops');
