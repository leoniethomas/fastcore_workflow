...
sampling_files = split(string(ls(path_to_sampling_data)));
...
load(path_to_model_to_analyse + filesep +   model_id + "_workspace_cond_models.mat") % load the condition specific models created with rFASTCORMICS
...
exp = exp.add_sampling_to_fastcore_experiment(sampling_files,1);
...
stats = exp.diff_flux_testing(["Cont\_NO",...
